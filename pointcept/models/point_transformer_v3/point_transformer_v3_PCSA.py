...
from .PCSA import PCSA,sort,get_basis
from .z_order import xyz2key
from .fps import fps


class Group(nn.Module):  # 使用 torch_cluster 的 FPS + KNN
...
@MODELS.register_module("PTv3-pgst")
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.num_stages):
            ...
            sub_U = get_basis(
                sub_center.reshape(B * sub_group_num, sub_group_size, 3)
            ).reshape(B, sub_group_num, sub_group_size, sub_group_size)
            if i > 0:
